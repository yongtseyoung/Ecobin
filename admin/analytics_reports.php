 ...
 * Generate, view, and export analytical reports
 */
...
function generateReportData($type, $start_date, $end_date) {
    ...
    switch($type) {
        ...
        case 'environmental':
            $total_weight = getOne("
                SELECT COALESCE(SUM(cr.total_weight), 0) as total
                FROM tasks t
                LEFT JOIN collection_reports cr ON cr.task_id = t.task_id
                WHERE t.status = 'completed'
                AND t.task_type = 'collection'
            ...
            $data = [
                ...
                'trees_equivalent' => $total_weight / 17,
                'energy_saved' => $total_weight * 2,
                'water_saved' => $total_weight * 3.5,
                'landfill_diverted' => $total_weight,
                'daily_breakdown' => getAll("
                    SELECT 
                        DATE(t.completed_at) as date,
                        COALESCE(SUM(cr.total_weight), 0) as weight,
                        COALESCE(SUM(cr.total_weight), 0) * 0.5 as co2_saved
                    FROM tasks t
                    LEFT JOIN collection_reports cr ON cr.task_id = t.task_id
                    WHERE t.status = 'completed'
                    AND t.task_type = 'collection'
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    switch($report_type) {
        ...
        case 'environmental':
            fputcsv($output, ['Environmental Impact Report']);
            fputcsv($output, ['Period', $start_date . ' to ' . $end_date]);
            fputcsv($output, []);
            fputcsv($output, ['Metric', 'Value']);
            fputcsv($output, ['Total Weight Collected (kg)', number_format($report_data['total_weight'], 2)]);
            fputcsv($output, ['CO2 Emissions Saved (kg)', number_format($report_data['co2_saved'], 2)]);
            fputcsv($output, ['Trees Equivalent', number_format($report_data['trees_equivalent'], 0)]);
            fputcsv($output, ['Energy Saved (kWh)', number_format($report_data['energy_saved'], 2)]);
            fputcsv($output, ['Water Saved (L)', number_format($report_data['water_saved'], 2)]);
            fputcsv($output, []);
            fputcsv($output, ['Date', 'Weight (kg)', 'CO2 Saved (kg)']);
            foreach ($report_data['daily_breakdown'] as $day) {
                fputcsv($output, [
                    $day['date'],
                    number_format($day['weight'], 2),
                    number_format($day['co2_saved'], 2)
                ]);
            }
            break;
    ...
    <div class="main-content">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="type" required>
                            ...
                            <option value="performance" <?php echo $report_type === 'performance' ? 'selected' : ''; ?>>Employee Performance</option>
                            <option value="bins" <?php echo $report_type === 'bins' ? 'selected' : ''; ?>>Bin Performance</option>
                            <option value="environmental" <?php echo $report_type === 'environmental' ? 'selected' : ''; ?>>Environmental Impact</option>
        ...
        <?php if ($report_data): ?>
            <div class="report-display">
                ...
                <?php elseif ($report_type === 'environmental'): ?>
                    ...
                    <div class="report-section">
                        ...
                        <div class="impact-grid">
                            <div class="impact-box">
                                <div class="icon">🌱</div>
                                <div class="value"><?php echo number_format($report_data['co2_saved'], 1); ?> kg</div>
                                <div class="label">CO₂ Emissions Saved</div>
                            </div>
                            <div class="impact-box">
                                <div class="icon">🌳</div>
                                <div class="value"><?php echo number_format($report_data['trees_equivalent'], 0); ?></div>
                                <div class="label">Trees Equivalent</div>
                            </div>
                            <div class="impact-box">
                                <div class="icon">⚡</div>
                                <div class="value"><?php echo number_format($report_data['energy_saved'], 1); ?> kWh</div>
                                <div class="label">Energy Saved</div>
                            </div>
                            <div class="impact-box">
                                <div class="icon">💧</div>
                                <div class="value"><?php echo number_format($report_data['water_saved'], 1); ?> L</div>
                                <div class="label">Water Saved</div>
                            </div>
                            <div class="impact-box">
                                <div class="icon">♻️</div>
                                <div class="value"><?php echo number_format($report_data['landfill_diverted'], 1); ?> kg</div>
                                <div class="label">Waste Diverted</div>
                            </div>
                            <div class="impact-box">
                                <div class="icon">⚖️</div>
                                <div class="value"><?php echo number_format($report_data['total_weight'], 1); ?> kg</div>
                    ...
                    <div class="report-section">
                        <h3>📈 Daily Environmental Impact</h3>
                        <table class="data-table">
                            <thead>
                                <tr>
                                    <th>Date</th>
                            ...
                            <tbody>
                                <?php foreach ($report_data['daily_breakdown'] as $day): ?>
                                    <tr>
                                        ...
                                        <td><?php echo number_format($day['weight'], 1); ?></td>
                                        <td><?php echo number_format($day['co2_saved'], 1); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
